  ...
  const handleSubmit = async () => {
    ...
    if (splitType === 'equal') {
      ...
      if (Math.abs(total - amt) > 0.01) {
        addToast(`مجموع المبالغ (${total.toFixed(2)}) لا يساوي الإجمالي (${amt.toFixed(2)}).`, 'error');
        return;
      }
      splits = selectedMembers.map(uid => ({ uid, amount: parseFloat(customAmounts[uid] || '0') }));
    }
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {members.map(m => (
                <div key={m.uid} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${
                ...
                onClick={() => toggleMember(m.uid)}>
                  <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 ${
                    selectedMembers.includes(m.uid) ? 'bg-emerald-500 border-emerald-500' : 'border-slate-600'
                  }`}>
                    {selectedMembers.includes(m.uid) && (
                  ...
                  {splitType === 'equal' && selectedMembers.includes(m.uid) && (
                    <span className="text-slate-400 text-xs">{equalShare} $</span>
                  )}
            ...
            {splitType === 'custom' && amount && (
              <p className={`text-xs mt-2 ${Math.abs(customTotal - parseFloat(amount)) < 0.01 ? 'text-emerald-400' : 'text-red-400'}`}>
                المجموع: {customTotal.toFixed(2)} $ من أصل {parseFloat(amount).toFixed(2)} $
              </p>
            )}
          </div>
          ...
          >
            {loading ? <LoadingSpinner size="sm" /> : 'إضافة المصروف'}
          </button>
        </div>
      </div>
    </div>
